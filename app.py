...
async def forward_request(
    ...
    logger.debug(f"Заголовки запроса: {dict(list(headers.items())[:5])}")
    ...
    async with httpx.AsyncClient(timeout=300.0) as client:
        try:
            ...
            # Сохраняем оригинальные названия заголовков (case-sensitive)
            for key, value in response.headers.items():
                key_lower = key.lower()
                if key_lower not in headers_to_exclude:
                    # Сохраняем оригинальное имя заголовка для важных заголовков ElevenLabs
                    response_headers[key] = value
            
            # Убеждаемся, что Content-Type присутствует и корректен
            ...
            logger.debug(f"Ответ от ElevenLabs: статус {response.status_code}, content-type: {content_type}")
            logger.debug(f"Размер ответа: {len(response.content)} байт")
            
            # Если ответ с ошибкой от ElevenLabs, передаем его как есть
            if response.status_code >= 400:
                logger.warning(f"Ошибка от ElevenLabs API: {response.status_code}")
                logger.debug(f"Тело ошибки: {response.content[:200]}")
                return Response(
